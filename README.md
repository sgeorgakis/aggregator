...
Optional request parameters are `date` (*yyyyMMdd* format), `app` and `level`.
...
If an error occurs while sending an event, the connection will close and be re-established with the next event.
...
The thread is started when the module starts running and is stopped when the module is stopped as well.
...
As the `thrift-client` module starts generating events at startup, a different implementation was used for testing.
The application waits for a method call to generate a logging event, instead of starting generating in intervals.
A simple implementation of a server was used to verify that the messages are properly sent.
This was achieved by using different profiles for running and testing the application.

The `thrift-server` module uses an embedded `Apache Kafka` instance and a simple implementation of a client and a consumer to verify the correct reception and forwarding of the logging events.
The embedded `Apache Kafka` instance binds a random port at startup.

The `kafka-consumer` module uses an embedded `Apache Kafka` instance, as well as an embedded `Apache Cassandra` instance.
The `Apache Cassandra` instance is available at `9142` port and the schema is initialized at startup, using a `cql` script located in the `test resources` folder.
A simple producer implementation is used to generate the logging events and it is verified that they are properly consumed and persisted.
...
Nevertheless, this exercise gave me the opportunity to explore and experiment with these technologies.

I decided to use the simple server implementation `Apache Thrift` offers and do not focus on security in any module or service configuration to focus more in the assignment implementation. For the same reason there is no error handling in the endpoint the `kafka-consumer` module exposes to verify the persistence of the logging events.

### Profiles
There are only 2 profiles used in the system, `dev` and `test`.
I decided not to use a `prod` profile, as this is not a production ready system.

### Apache Cassandra Integration
`Spring` offers a simple integration with `Apache Cassandra` with its `Spring Data` module.
As its behavior was a little faulty (in the **save** query more specifically),
I decided to use the `Mapper` class the `cassandra-driver-mapping` module offers.
